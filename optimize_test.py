...
def execute_ir(input,output,opt,stdin,stdout,testout):
    ...
    result = execute(["clang-18",output,"-c","-o","tmp.o","-w"])
    if(result.returncode != 0):
        print("\033[93mOutPut Error on \033[0m"+input)
        return 0
        
    result = execute(["clang-18","-static","tmp.o","lib/libsysy_x86.a"])
    if(result.returncode != 0):
...
input_folders = ["basic_mem2reg","mem2reg","eliUnreachablebb","adce","scalar_cse","sccp"]
