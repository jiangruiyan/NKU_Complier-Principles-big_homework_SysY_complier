...
namespace BE::RV64
{
        ...
        switch (opType)
        {
            ...
            case OpType::I:
            {
                ...
                if (isMemInst(inst->op))
                {
                    ...
                    else if (inst->use_ops && inst->fiop)
    ...
    void CodeGen::printOperand(const Register& reg)
    {
        ...
        {
            // 防止 reg.dt 为空导致崩溃
            const char* dtName = (reg.dt ? reg.dt->toString().c_str() : "unk");
            out_ << "v_" << reg.rId << "_" << dtName;
            return;
        }
    ...
    // 新增：覆盖伪移动打印，确保 src/dest 判空
    void BE::RV64::CodeGen::printPseudoMove(BE::MoveInst* inst)
    {
        if (!inst)
        {
            out_ << "# null-move";
        ...
        // 目的操作数（如果是寄存器，打印寄存器；否则占位）
        if (inst->dest && inst->dest->ot == BE::Operand::Type::REG)
            printOperand(static_cast<BE::RegOperand*>(inst->dest)->reg);
        else
            out_ << "x0";

        out_ << ", ";

        // 源操作数：优先寄存器；否则打印常量/帧索引；皆为空则占位
        if (inst->src)
