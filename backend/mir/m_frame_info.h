    ...
    class MFrameInfo
    {
      public:
        enum class ObjectKind
        {
            ...
            IncomingArg  // incoming args from caller's stack
        };
      ...
      private:
        ...
        std::unordered_map<int, FrameObject>    incomingArgObjs_;    // stack arg index -> object
      ...
      public:
        ...
        void createIncomingArgObject(int argIndex, int stackOffset)
        {
            // Incoming args have positive offset (relative to sp after prologue)
            // They are stored by caller before the call
            incomingArgObjs_[argIndex] = FrameObject{8, 8, stackOffset, ObjectKind::IncomingArg};
        }

        int getIncomingArgOffset(int argIndex) const
        {
            auto it = incomingArgObjs_.find(argIndex);
            if (it == incomingArgObjs_.end()) return -1;
